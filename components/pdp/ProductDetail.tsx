  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className="flex items-center gap-0.5">
        {[1, 2, 3, 4, 5].map((s) => (
          <Star key={s} size={14} className={s <= Math.round(rating) ? "fill-gold text-gold" : "fill-ink/10 text-ink/10"} />
        ))}
      </div>
      <span className="text-sm text-ink/40 tabular-nums">{rating.toFixed(1)} ({count} reviews)</span>
    </div>
  );
}
...
function Accordion({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="border-b border-line-light">
      ...
      {open && <div className="pb-4 text-sm text-ink/50 leading-relaxed">{children}</div>}
    </div>
  );
}
...
export function ProductDetail({ jersey, related }: ProductDetailProps) {
  ...
  return (
    <>
      {/* Breadcrumbs */}
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 py-4 text-xs text-ink/40 flex items-center gap-1.5">
        <Link href="/" className="hover:text-brand transition-colors">Home</Link>
        <span>/</span>
        <Link href="/shop" className="hover:text-brand transition-colors">Shop</Link>
        <span>/</span>
        <span className="text-ink/70 font-medium truncate">{jersey.name}</span>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-24">
        <div className="grid lg:grid-cols-2 gap-10 xl:gap-16">
          ...
          <div className="space-y-6">
            ...
            {jersey.badges.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {jersey.badges.map((b) => {
                  ...
                  return <span key={b} className={`text-xs font-bold uppercase tracking-wide px-2 py-1 rounded-md ${colors[b] ?? "bg-ink/10 text-ink/60"}`}>{b}</span>;
                })}
              </div>
            )}
            ...
            {/* Name */}
            <div>
              <p className="text-sm text-ink/40 mb-1">{getCountryFlag(jersey.countryCode)} {jersey.country} · {jersey.maker} · {jersey.type}</p>
              <h1 className="font-archivo font-black uppercase text-ink text-2xl sm:text-3xl tracking-tight leading-tight">{jersey.name}</h1>
              <p className="text-sm text-ink/40 mt-1">{jersey.colorway}</p>
            </div>
            ...
            <div className="flex items-center justify-between">
              <p className="text-3xl font-bold text-ink tabular-nums">
                ...
                <span className="text-sm text-ink/40 font-normal ml-1">CAD</span>
              </p>
              <StarRating rating={jersey.rating} count={jersey.reviewCount} />
            </div>

            <p className="text-sm text-ink/50 leading-relaxed">{jersey.description}</p>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 border border-line-light rounded-lg w-fit">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="p-2.5 hover:bg-ink/5 rounded-l-lg transition-colors text-ink" aria-label="Decrease quantity"><Minus size={14} /></button>
                <span className="px-3 text-sm font-semibold text-ink tabular-nums min-w-[32px] text-center">{qty}</span>
                <button onClick={() => setQty(qty + 1)} className="p-2.5 hover:bg-ink/5 rounded-r-lg transition-colors text-ink" aria-label="Increase quantity"><Plus size={14} /></button>
              </div>
            </div>

            {(qty > 1 || printing) && (
              <p className="text-sm text-ink/40">
                Total: <span className="font-bold text-ink tabular-nums">{formatCAD(totalPrice)}</span>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button onClick={handleBuyNow} className="flex-1 flex items-center justify-center gap-2 border-2 border-ink/20 text-ink font-archivo font-bold uppercase tracking-wide text-sm py-3.5 rounded-xl hover:bg-ink/5 transition-colors min-h-[44px]">
                <Zap size={16} /> Buy Now
              </button>
            </div>
            ...
            <ul className="space-y-1.5">
              {jersey.highlights.map((h) => (
                <li key={h} className="flex items-center gap-2 text-sm text-ink/50">
            ...
            <div className="border-t border-line-light pt-2">
              ...
              <Accordion title="Shipping & Returns">
                <div className="space-y-2">
                  ...
                  <p><strong className="text-ink/70">Standard:</strong> 5–8 business days · Free on orders over $150, otherwise $14.99.</p>
                  <p><strong className="text-ink/70">Express:</strong> 2–3 business days · $24.99.</p>
                  <p>30-day returns on unworn, unaltered jerseys. Personalized (printed) jerseys are <strong className="text-ink/70">final sale</strong> and cannot be returned.</p>
                </div>
              </Accordion>
            </div>
          </div>
        ...
        <section className="mt-16 pt-10 border-t border-line-light" id="reviews">
          <h2 className="font-archivo font-black uppercase text-ink text-2xl tracking-tight mb-8">Customer Reviews</h2>
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center md:text-left">
              <p className="text-6xl font-black text-ink tabular-nums">{jersey.rating.toFixed(1)}</p>
              <div className="flex items-center justify-center md:justify-start gap-1 my-2">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star key={s} size={18} className={s <= Math.round(jersey.rating) ? "fill-gold text-gold" : "fill-ink/10 text-ink/10"} />
                ))}
              </div>
              <p className="text-sm text-ink/40">Based on {jersey.reviewCount} reviews</p>
              <div className="mt-4 space-y-1.5">
                {ratingDist.map((r) => (
                  <div key={r.stars} className="flex items-center gap-2 text-xs">
                    <span className="text-ink/40 w-8 text-right tabular-nums">{r.stars}★</span>
                    ...
                    <span className="text-ink/40 w-7 tabular-nums">{r.pct}%</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div key={i} className="border-b border-line-light pb-6 last:border-0">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex gap-0.5">
                      {[1, 2, 3, 4, 5].map((s) => (
                        <Star key={s} size={12} className={s <= r.rating ? "fill-gold text-gold" : "fill-ink/10 text-ink/10"} />
                      ))}
                    </div>
                  </div>
                  <p className="text-sm text-ink/50 leading-relaxed">{r.body}</p>
                </div>
              ))}
        ...
        {related.length > 0 && (
          <section className="mt-16 pt-10 border-t border-line-light">
            <h2 className="font-archivo font-black uppercase text-ink text-2xl tracking-tight mb-6">You Might Also Like</h2>
      ...
      {/* Sticky mobile bar */}
      <div className="lg:hidden fixed bottom-0 inset-x-0 bg-paper border-t border-line-light p-4 flex gap-3 z-40 shadow-2xl">
        <div className="flex-1">
          <p className="text-xs text-ink/40">{jersey.name}</p>
